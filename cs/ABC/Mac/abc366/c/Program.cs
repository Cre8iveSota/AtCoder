...
namespace x
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Dictionary<int, int> typeNum = new Dictionary<int, int>();
            int[] expectedInputData1 = new int[Q];
            int[] expectedInputData2 = new int[Q];
            int currentTypesCnt = 0;
            for (int i = 0; i < Q; i++)
            {
                string[] line = Console.ReadLine().Split(' ');
                expectedInputData1[i] = int.Parse(line[0]);
                expectedInputData2[i] = int.Parse(line[0]) == 3 ? 0 : int.Parse(line[1]);
                if (expectedInputData1[i] == 1 && typeNum.ContainsKey(expectedInputData2[i]))
                {
                    typeNum[expectedInputData2[i]]++;
                }
                else if (expectedInputData1[i] == 1 && !typeNum.ContainsKey(expectedInputData2[i]))
                {
                    typeNum.Add(expectedInputData2[i], 1);
                    currentTypesCnt++;
                }
                else if (expectedInputData1[i] == 2)
                {
                    typeNum[expectedInputData2[i]]--;
                    if (typeNum[expectedInputData2[i]] == 0)
                    {
                        typeNum.Remove(expectedInputData2[i]);
                        currentTypesCnt--;
                    }
                }
                else
                {
                    Console.WriteLine(currentTypesCnt);
                }
            }
        }
    }
