...
namespace x
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int[] NTP = Utils.IntArray1In();
            int N = NTP[0];
            int T = NTP[1];
            int P = NTP[2];

            int[] L = Utils.IntArray1In();
            int cntDay = 0;
            int cntNum = 0;
            int ex = L.Where(i => i >= T).ToArray().Length;
            if (L.Where(i => i >= T).ToArray().Length >= P)
            {
                Console.WriteLine(0);
            }
            else
            {
                while (cntNum < P)
                {
                    L = L.Select(i => i + 1).ToArray();
                    cntNum = L.Where(i => i >= T).ToArray().Length;
                    cntDay++;
                }
