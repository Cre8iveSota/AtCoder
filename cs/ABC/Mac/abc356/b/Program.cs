...
namespace x
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int[] line0 = Utils.IntArray1In();
            int N = line0[0];
            int M = line0[1];
            int[] line1 = Utils.IntArray1In();
            int[][] meals = Utils.IntJaggedArrayIn(N);

            bool ok = true;
            for (int i = 0; i < M; i++)
            {
                if (!IsOverAimOfAday(i))
                {
                    ok = false;
                    break;
                }
            ...
            bool IsOverAimOfAday(int index)
            {
                int totalCal = 0;
                for (int i = 0; i < N; i++)
                {
                    totalCal += meals[i][index];
                }
                if (totalCal >= line1[index])
                {
                    return true;
                }
                else
