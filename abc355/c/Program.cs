...
namespace x
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int[] nt = Utils.IntArray1In();
            int[] A = Utils.IntArray1In();
            int N = nt[0];
            int T = nt[1];

            // line
            // number is gonna be incriment until the max line which index is N-1 && start from 

        }

        static bool DiagonalChecker(int N, int[] A)
        {
            int[] bingoList = new int[2];
            for (int i = 1; i <= N; i++)
            {
                bingoList[i - 1] =
            }
