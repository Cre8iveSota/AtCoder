...
namespace x
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int[] nq = Utils.IntArray1In();
            int n = nq[0];
            int q = nq[1];

            int[] targetT = Utils.IntArray1In();
            Dictionary<int, int> teethtreat = new Dictionary<int, int>();
            foreach (int i in targetT)
            {
                if (teethtreat.ContainsKey(i))
                {
                    teethtreat[i] += 1;
                }
                else
            ...
            int ans = n;
            foreach (int i in teethtreat.Values)
            {
                if (i % 2 == 1)
                {
                    ans--;
