...
namespace x
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                for (long i = M / N; i * N <= M || calSum(i) <= M; i++)
                {
                    ans = i;
                }
            ...
            long calSum(long maxBudget)
            {
                long minEven = A.Where(n => n >= maxBudget).Min();
                long index = Array.IndexOf(A, minEven);
                long overdmoney = (A.Length - index) * maxBudget;
                long AdashSum = A.Where(x => x < minEven).ToArray().Sum();
                return overdmoney + AdashSum;
            }
        }
    }
