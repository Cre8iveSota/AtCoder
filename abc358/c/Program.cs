...
namespace x
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int[] NM = Utils.IntArray1In();
            int N = NM[0];
            int M = NM[1];
            string[] S = new string[N];
            int[] countDicSum = new int[N];
            for (int i = 0; i < N; i++)
            {
                S[i] = Utils.StringIn();
            }
            Dictionary<int, int>[] store = new Dictionary<int, int>[N];
            for (int i = 0; i < N; i++)
            {
                for (int j = 0; j < M; j++)
                {
                    if (S[i].Substring(j, 1) == "o")
                    {
                        store[i].Add(j, 1);
                    }
                    else
                ...
                countDicSum[i] = store[i].Values.Sum();
            }

            int targetIndex = Array.IndexOf(countDicSum, countDicSum.Max());
