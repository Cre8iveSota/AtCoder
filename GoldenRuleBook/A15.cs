...
class Program
{
    static void Main(string[] args)
    {
        ...
        // なぜsample1の引数に空白が入っているんだぁぁぁぁぁ! Trim()が必要なんて、、それはハマるわ
        long[] A = Console.ReadLine().Trim().Split(' ').Select(i => long.Parse(i)).ToArray();

        long[] B = A.Distinct().OrderBy(i => i).ToArray();

        long[] C = new long[N];
        for (long i = 0; i < N; i++)
        {
            long index = Array.BinarySearch(B, A[i]);
            C[i] = index + 1;
        }
